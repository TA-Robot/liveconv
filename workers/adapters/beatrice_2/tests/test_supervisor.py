...
def profile() -> WorkerProfile:
    return WorkerProfile(
        profile_id="beatrice-2.conformance",
...
def test_supervisor_round_trip_health_partial_drain_and_close() -> None:
    async def scenario() -> None:
        supervisor = WorkerSupervisor(profile())
        try:
            ready = await supervisor.start()
            assert ready.implementation_revision == "beatrice-2-test-backend"
...
def test_subprocess_stdout_contains_protocol_lines_only() -> None:
    async def scenario() -> None:
        supervisor = WorkerSupervisor(profile())
        try:
            await supervisor.start()
...
def test_subprocess_rejects_unbound_configuration_hash() -> None:
    async def scenario() -> None:
        supervisor = WorkerSupervisor(
            replace(profile(), configuration_hash="sha256:" + "5" * 64)
        )
        try:
            with pytest.raises(WorkerRuntimeError, match="initialization failed"):
                await supervisor.start()
        finally:
            await supervisor.close()
