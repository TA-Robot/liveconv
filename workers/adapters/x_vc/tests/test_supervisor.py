...
from workers.adapters.x_vc.backend import DeterministicTestBackend
...
def profile() -> WorkerProfile:
    ...
    return WorkerProfile(
        profile_id="x-vc.deterministic-conformance",
...
def test_supervisor_round_trip_health_end_drain_and_stdout_purity() -> None:
    async def scenario() -> None:
        supervisor = WorkerSupervisor(profile())
        try:
            ready = await supervisor.start()
            assert ready.implementation_revision == (
                "x-vc-deterministic-test-backend-v1"
            )
            ...
            end_task = asyncio.create_task(
                supervisor.end_generation(source.generation_id)
            )
            output = await supervisor.next_output()
            await end_task
            assert output.unpack_samples() == pytest.approx((-0.125,) * 960)
...
def test_supervisor_cancel_clears_pending_audio_and_stale_output() -> None:
    async def scenario() -> None:
        supervisor = WorkerSupervisor(profile())
        try:
...
def test_supervisor_rejects_configuration_identity_mismatch() -> None:
    async def scenario() -> None:
        supervisor = WorkerSupervisor(
            replace(profile(), configuration_hash="sha256:" + "e" * 64)
        )
        try:
            with pytest.raises(WorkerRuntimeError) as caught:
                await supervisor.start()
