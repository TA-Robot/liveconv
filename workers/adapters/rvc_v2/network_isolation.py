...
def deny_non_unix_sockets() -> None:
    """Irreversibly deny every socket domain except AF_UNIX.

    The filter is installed by the worker entrypoint before real-model startup,
    and is inherited by the inference thread and any subprocess descendants.
    """
...
def require_non_unix_socket_denial() -> None:
    """Refuse real model startup unless the OS network filter is active."""

    if not _installed:
        raise RuntimeError("RVC OS network isolation is not active")
