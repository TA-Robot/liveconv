...
def experiment_policy(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        ...
        return {
            ...
            "slug": "exp134",
            "candidate_id": "cv12-real-teacher-output-window48",
            "candidate_name": (
                "EXP-134 / full-output teacher / audited start-middle-end windows"
            ),
            "run_kind": "EXP-134 X-VC model-window output teacher evaluation",
            "result_kind": "liveconv-exp134-xvc-real-teacher-output-window48/v1",
            "question": (
                "Does selecting the exact 2.4-second model windows improve broad "
                "content stability without adding repetition?"
            ),
            "independent_variable": (
                "the 21 Hadou teacher inputs: EXP-130 full utterances selected by "
                "official full-text kana versus seven unique audited start, middle, "
                "and end 2.4-second windows selected by actual window-ASR 1/2/3-gram "
                "coverage; CV24, JVS3, count/share, full-output objective, standard "
                "rows, updates, control69 LoRA, LR, seed, and target stay fixed"
            ),
        }
    if (
    ...
    if (
        ...
        return {
            ...
            "slug": "exp124",
            "candidate_id": "cv12-real-teacher-output-multidomain48",
            "candidate_name": (
                "EXP-124 / full-output teacher / CV24 + Hadou21 + JVS3"
            ),
            "run_kind": "EXP-124 X-VC multi-domain output teacher evaluation",
            "result_kind": (
                "liveconv-exp124-xvc-real-teacher-output-multidomain48/v1"
            ),
            "question": (
                "Does cross-corpus teacher-source composition improve robust "
                "generalization without changing source count or teacher share?"
            ),
            "independent_variable": (
                "48-source teacher pool composition: EXP-116 Common Voice 48 "
                "versus Common Voice 24, disjoint Hadou 21, and official JVS 3; "
                "209 teacher positions, full-output objective, 835 standard rows, "
                "updates, control69 standard LoRA, LR, seed, and target stay fixed"
            ),
        }
    if (
...
def _load_fixed_multidomain_pool(
    ...
    try:
        ...
        raise RoleMixError("fixed multi-domain pool is not valid JSON") from error
    items = value.get("items") if isinstance(value, dict) else None
    if (
        not isinstance(value, dict)
        or value.get("kind") != kind
    ...
    for item in items:
        ...
        if (
            ...
            raise RoleMixError("fixed multi-domain pool row drifted")
        if domain == "commonvoice":
            if client in cv_clients:
                raise RoleMixError("multi-domain Common Voice speaker repeated")
            cv_clients.add(client)
        ids.add(identifier)
        files.add(filename)
        domains[domain] += 1
...
def validate_inputs(
    ...
    if arguments.training_policy in {
        ...
        teacher_clients = {
            item["client_id_sha256"]
            for item in teacher_pool["items"]
            if item.get("domain", "commonvoice") == "commonvoice"
        }
