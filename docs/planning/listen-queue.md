...
| EXP-317 X-VC CV32 replacement at 170 updates | EXP-238 versus the same 170-row corpus composition with its first 32 Common Voice source/teacher tuples replaced by the admitted CV32 rows | after hearing returns, compare naturalness and target fit only if external7 preserves corruption/content stability | admitted next lane; holds 170 updates and every position's real Amitaro assignment fixed, reuses existing source/teacher bytes, and changes only 32 Common Voice data tuples |
