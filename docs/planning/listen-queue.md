...
Current GPU lane (2026-08-14 14:40 UTC): EXP-266 stopped before full training
because both real smoke rows already exceeded its latent speaker margin.
EXP-267 now keeps EXP-238's same-content teacher for semantic/mel reconstruction
but uses the assigned real Amitaro window for the existing global speaker
condition and speaker MSE. A two-row finite smoke gates one 170-update run; all
hearing claims remain open.
...
| EXP-203--207 X-VC unpaired human factorization | Base, control69, and alignment-free Hadou-content / unrelated-Amitaro-speaker factorization across external7, fresh48, Hadou31, stress60, and balanced JSUT24 | after hearing returns: optional diagnosis of whether the Hadou/category gains are audible despite the tempo and corruption stop; do not nominate this arm | 850 WAVs; Hadou and some category rows improve, but tempo1.2 is `0/4/4` and fresh48 gains a new 223-character consensus repetition on `cv39028774f`; this exact objective is technically rejected, closed, and unselected |
| EXP-208--212 X-VC final-WAV content cycle | Base, control69, and unpaired human factorization with the content loss moved from an internal head to a frozen-Whisper cycle on the converted WAV, across all five fixed surfaces | after hearing returns: optional diagnosis of the broad stress/Hadou effect; do not nominate this arm because the corruption stop fired | 850 WAVs; Hadou and stress60 improve, but tempo1.2 remains worse and fresh48 adds a 334-character consensus phrase loop on low-information `cv44571685f`; this exact objective is technically rejected, closed, and unselected |
| EXP-213--217 X-VC cross-corpus output cycle | Base, control69, and the fixed final-WAV cycle trained on CV48 + disjoint JSUT85 + JVS3 + Hadou34 sources, across all five fixed surfaces | after hearing returns: judge naturalness, target identity, and whether the Hadou/noise gain survives without a tempo or ordinary-JSUT quality cost | 850 WAVs; no candidate-added consensus gross row; Hadou and noise improve strongly, external/fresh are near mixed, while tempo1.2 and ordinary JSUT remain worse on the auxiliary content diagnostic; technically viable, unheard, and unselected |
| EXP-218--222 X-VC contrastive output cycle | Base, control69, and the exact cross-corpus method with pointwise MSE replaced by one fixed source-versus-next-row InfoNCE content objective | optional diagnosis after hearing returns; do not nominate this arm because the broad-content stop fired | 850 WAVs; no candidate-added consensus gross row and tempo1.2 improves, but fresh48 common-stable content regresses `0.155 -> 0.226` and JSUT24 `0.115 -> 0.158`; contrastive temperature/negative/mining/blend neighbors are closed |
| EXP-223--227 X-VC discrete output cycle | Base, control69, and the exact cross-corpus method with final-WAV content replaced by frozen 16,384-way WhisperVQ token classification | optional diagnosis after hearing returns; do not nominate this arm because the broad-JSUT stop fired | 850 WAVs; no candidate-added consensus gross row; external/Hadou/noise improve, but JSUT24 common-stable source distance regresses `0.115 -> 0.152` and the training token objective diverges; discrete/content-cycle neighbors are closed |
| EXP-228--232 X-VC content/voice PCGrad | Base, control69, and the exact cross-corpus MSE method with per-row content-versus-voice gradient surgery | optional diagnosis after hearing returns; do not nominate this arm because the broad-JSUT stop fired | 850 WAVs; conflicts occurred on 92/170 rows and no candidate-added gross row appeared, but JSUT24 regressed `0.115 -> 0.146`; weights, grouping, projection, and adjacent optimizer-surgery points are closed |
| EXP-233--237 X-VC speaker-path voice overlay | Base, control69, and merged control69 plus a voice-only seven-AdaLN overlay across the five fixed surfaces | optional naturalness/identity diagnosis after hearing returns; do not nominate because broad content retention failed | 850 WAVs; no candidate-added gross row and Hadou improves, but fresh48, stress60, and JSUT24 regress; speaker-path rank/scope/weight neighbors are closed |
| EXP-238--242 X-VC source-aligned pseudoparallel | Base, control69, and control69 initialized then retrained on broad cross-corpus sources with same-content frozen-control69 targets and original real Amitaro discriminator references | after hearing returns: prioritize this technical survivor for naturalness, target-voice fit, and whether its broad content stability is audible | 850 WAVs; no candidate-added gross row; external exact-ties, fresh/Hadou/stress improve on common-stable content, tempo ties, and JSUT has 20/22 ties plus one win/one loss; technically retained but not a keeper or winner |
| EXP-243 expanded stress matrix | Source, target, base, control69, and EXP-238 across 16 disjoint speakers, four text-length bands, and nine symmetric clean/noise/silence/tempo/pitch conditions | after hearing returns: use representative wins, ties, and losses to judge naturalness and target-voice fit; do not optimize one row | 720 WAVs; no gross row; common-stable source distance improved overall and especially on long rows, while 13 losses were distributed across eight speakers and no single condition explained them; stable, unheard, and unselected |
| EXP-244--250 X-VC SRC4VC source substitution | EXP-238 and the JSUT85-to-SRC4VC85 retrain across external7, fresh48, Hadou31, stress60, JSUT24, expanded144, plus a separate heldout SRC4VC30 surface | after hearing returns: optional naturalness and target-voice-fit comparison; do not nominate from ASR or speaker embeddings | no candidate-added gross row; direct common-stable content improves external/Hadou but regresses fresh/stress/expanded and ties JSUT, so the exact substitution is closed for broad content improvement but remains an unheard changed-audio alternative |
| EXP-252--258 X-VC final-WAV speaker supervision | EXP-238 versus direct final-WAV ERes2Net-supervised LoRA69 across external7, fresh48, Hadou31, stress60, JSUT24, and expanded144 | after hearing returns: optional target-identity/naturalness diagnosis only; do not nominate because broad content retention failed | all 314 candidate WAVs changed and no new gross row appeared; independent ECAPA target cosine improved on all six surfaces (`+0.007629`, 232W/82L), but fresh/stress content and decoder stability regressed; exact LoRA69 method and speaker-weight neighbors are closed |
...
| EXP-267--272 X-VC real speaker-condition target | EXP-238 versus the one target-contract change that keeps its same-content reconstruction target but uses authorized real Amitaro for X-VC's global speaker condition and existing speaker MSE | after hearing returns: judge target identity, naturalness, and content across the full set; do not nominate from ASR | 170-update training and external7 are complete; 7/7 candidate WAVs changed with zero added gross, but the five common stable rows are 0W/4T/1L, so broad fixed-surface rendering is in progress and the arm remains unselected |
