...
| EXP-181--182 X-VC parameter anchor | Base, control69, and the exact EXP-163 method plus a light control69 L2-SP anchor on external7 and fresh48 | after hearing returns: judge whether the constrained adapter improves naturalness or identity without flattening conversion | 275 WAVs add no consensus gross row; fresh36 regressed source-relative content but was near-neutral to known text, so Hadou31 tests whether hard repair survived; no automatic winner |
