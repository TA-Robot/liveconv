...
zero added gross and five jointly stable source-relative ties, but decoder
instability still increased `1 -> 2` on `cv45195640`; broad rendering stopped.
...
| EXP-326--332 X-VC source-speaker GRL | EXP-325 versus the identical 85-speaker x2 contract with only a training-only post-converter source-speaker GRL, across external7 and six fixed broad surfaces | after hearing returns: optional naturalness and target-voice comparison; do not nominate from machine results | 3,440 matched listener WAVs; broad 337-row join `15W/303T/19L`, source-relative mean `0.609039 -> 0.628134`, instability `70 -> 67` but 7 new/10 recovered rows, gross `2 -> 2` with no added gross; expanded144 rejects generic broad-content improvement and closes GRL neighbors |
