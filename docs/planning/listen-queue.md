...
| EXP-171--173 X-VC JSUT retention | Base, control69, and the category-balanced JSUT-retention EMA branch across external7, fresh48, and Hadou31 | after hearing returns: optional diagnosis of ordinary-row naturalness only; do not promote this checkpoint | method is technically rejected because it added a `RECITATION324_138` repeated-number gross loop; stress60 and JSUT24 were stopped |
| EXP-176--178 X-VC paired PCGrad | Base, control69, and hard-repair/retention gradient surgery across external7, fresh48, and Hadou31 | v4 two-decode-consensus/common-stable machine reassessment first; after hearing returns judge naturalness and identity without treating ASR as a winner | beam-5-only gross rejection is retracted: the 2.4 s source itself hallucinated 428 characters while greedy gave short plausible text and matched candidate/control; checkpoint remains unselected pending v4 |
