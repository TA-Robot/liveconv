...
| EXP-259 X-VC speaker-condition calibrator | Frozen EXP-238 plus one zero-init 192-value target-speaker condition delta, initially on external7 | pending render; after hearing returns judge identity/naturalness only if broad content survives | admitted one conditioning-path point; no PCGrad, speaker7, loss-weight, data, scope, LR, or horizon neighbor |
